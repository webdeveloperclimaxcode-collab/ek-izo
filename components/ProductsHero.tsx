...
export default function ProductsHero({ searchQuery, onSearchChange }: ProductsHeroProps) {
  const { t } = useLanguage();
  ...
  return (
    <section className="relative w-full h-[400px] flex items-center justify-center">
      ...
      <div className={`absolute inset-0 z-0 ${theme === "dark" ? "brightness-50" : ""}`}>
        <Image
          src="/assets/images/Products_page/hero.png"
      ...
      <div className="relative z-10 w-full px-6 2xl:px-20 max-w-5xl mx-auto">
        ...
        <h1 className="text-white text-[48px] md:text-[56px] font-bold text-center mb-8 tracking-tight">
          {t("productsPage.productLine")}
        </h1>

        {/* Search Bar */}
        <form onSubmit={handleSearch} className="products-hero-search relative max-w-3xl mx-auto">
          <div className="relative">
            <input
              type="text"
              placeholder={t("productsPage.searchProduct")}
              value={searchQuery}
              onChange={(e) => onSearchChange(e.target.value)}
              className={`w-full px-8 py-5 rounded-full text-lg focus:outline-none transition-colors ${
                theme === "dark"
                  ? "bg-[#FFFFFF]/20 text-white placeholder:text-white placeholder:opacity-100"
                  : "bg-[#FFFFFF]/40 text-[#1B2556] placeholder:text-[#1B2556]"
              }`}
            />
            <button
              type="submit"
